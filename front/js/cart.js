...
const showDatas = async () => {
  ...
  let idData = datas;
  for (let m = 0; m < idData.length; m++) {
    console.log(idData[m]._id);
    ...
    for (let u = 0; u < productInStorage.length; u++) {
      ...
      newH2.textContent = idData[m].name;
      newP1.textContent = productInStorage[u].color;
      newP2.textContent = idData[m].price + ` € `;
      newP3.textContent = ` Qté : ${productInStorage[u].quantity}`;
      newImg.src = idData[m].imageUrl;
      newImg.alt = idData[m].altTxt;
